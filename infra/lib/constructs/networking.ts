...
export class Networking extends Construct {
  ...
  constructor(scope: Construct, id: string, props: NetworkingProps) {
    ...
    //
    // In `internal` mode we can use Vpc.fromLookup (resolves at synth time).
    // In `generic` mode the VPC ID is a CFN token unresolvable at synth,
    // so we use fromVpcAttributes + Fn.getAzs().
    ...
    this.albSecurityGroup = new ec2.SecurityGroup(this, "AlbSg", {
      vpc: this.vpc,
      securityGroupName: `outline-alb-${config.envName}`,
      description: "Outline ALB - HTTP/HTTPS from internet",
    });
    ...
    this.appSecurityGroup = new ec2.SecurityGroup(this, "AppSg", {
      vpc: this.vpc,
      securityGroupName: `outline-app-${config.envName}`,
      description: "Outline ECS service - traffic from ALB only",
    });
    this.appSecurityGroup.addIngressRule(
      this.albSecurityGroup,
      ec2.Port.tcp(3000),
      "Outline app from ALB",
    );

    this.dbSecurityGroup = new ec2.SecurityGroup(this, "DbSg", {
      vpc: this.vpc,
      securityGroupName: `outline-db-${config.envName}`,
      description: "Outline RDS - access from ECS service",
    });
    ...
    this.redisSecurityGroup = new ec2.SecurityGroup(this, "RedisSg", {
      vpc: this.vpc,
      securityGroupName: `outline-redis-${config.envName}`,
      description: "Outline Redis - access from ECS service",
    });
    this.redisSecurityGroup.addIngressRule(
      this.appSecurityGroup,
      ec2.Port.tcp(6379),
      "Redis from ECS service",
    );
    ...
    this.alb = new elbv2.ApplicationLoadBalancer(this, "Alb", {
      ...
      loadBalancerName: `outline-${config.envName}`,
    ...
    this.appTargetGroup = new elbv2.ApplicationTargetGroup(this, "AppTg", {
      ...
      targetGroupName: `outline-app-${config.envName}`,
      healthCheck: {
        path: "/_health",
